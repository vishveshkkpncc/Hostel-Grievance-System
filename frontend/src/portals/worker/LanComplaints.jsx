...
function LanComplaints() {
  ...
  useEffect(() => {
    ...
    // Safety check – if something is wrong with auth, don't call the API
    if (!token || !user?.name) {
    ...
    fetch(`http://localhost:5000/api/complaints/worker/${encodeURIComponent(user.name)}`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        const lanComplaints = data
          .filter((c) => c.category === "Lan" && c.status !== "resolved")
          .sort((a, b) => {
            // Emergency first
        ...
        {complaints.length > 0 ? (
          complaints.map(complaint => (
            <div key={complaint._id} className="complaint">
              ...
              <Link to={`/worker/update-complaint/lan/${complaint._id}`} className="update-link">Update Progress</Link>
            </div>
           ))
         ) : (
